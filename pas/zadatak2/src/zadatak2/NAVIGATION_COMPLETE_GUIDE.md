...
```bash
ros2 pkg executables zadatak2 | grep -E "path_follower|a_star"
```

Očekivani output:
```
zadatak2 a_star_path_planner
zadatak2 path_follower
...
```
...
```bash
ros2 launch zadatak2 navigation_complete.launch.py
```
...
```bash
# AMCL lokalizacija
ros2 launch zadatak2 localization_complete_launch.py
```
